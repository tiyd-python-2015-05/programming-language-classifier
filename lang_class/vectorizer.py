...
class CodeVectorizer(TfidfVectorizer):
    def __init__(self):
        self.fit_X = None
        self.fit_Y = None
    ...
    def find_ave_brackets(self, X): # assumes all delimiters are matched
        if isinstance(X, list):
            final = []
            for item in X:
                final.append(self.find_ave_brackets(item))
            return final
    ...
    def process(self):
        self.lengths = self.find_ave_brackets(self.fit_X)
        self.punctuation = self.find_punctuation(self.fit_X)


    def transform(self, X):
        if self.fitted:
            ln = coo_matrix(np.matrix(self.find_ave_brackets(X))).transpose()
